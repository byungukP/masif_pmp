...
def crosscheck(pmp_df, res_id, chain_ids1, resname):
    ...
    assert data_chain == chain_ids1, \
        f"Mismatch: chain id mismatch between residues (res_id: {res_id}) from mesh vertices and pmp_dataset.csv\n \
            mesh vertices: {chain_ids1}\n \
            pmp_dataset.csv: {data_chain}"
    assert data_resname == resname, \
        f"Mismatch: residue name mismatch between residues (res_id: {res_id}) from mesh vertices and pmp_dataset.csv\n \
