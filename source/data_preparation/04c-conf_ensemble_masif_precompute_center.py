...
for ppi_pair_id in ppi_pair_list:
    ...
    center_list = [f for f in os.listdir(ply_dir) if f.startswith("Center_")]

    # loop for each cluster center
    for center in center_list:
        ply_file = os.path.join(ply_dir, center, f"/{ppi_pair_id}.ply")
        frame_ply_template = f"{ply_dir}/{center}" + "/{}_{}.ply"
        ...
        for pid in pids:
            try:
                ...
                print(f"{ppi_pair_id} {center} {pid} IndexError")
                continue

        my_precomp_dir = params['masif_precomputation_dir']+ppi_pair_id+f"/{center}/"
