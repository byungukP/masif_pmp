...
"""
masif_site_generate_vec_ensemble.py
: Generate fingerprint vectors for one or multiple proteins with pretrained MaSIF-site.
can handle structural ensemble generated by HTMD & clustered by CLoNe.
...
for ppi_pair_id in ppi_pair_ids:
    ...
    ppi_pair_dir = parent_in_dir + ppi_pair_id + "/"
    cluster_num = len([f for f in os.listdir(ppi_pair_dir) if f.startswith("Cluster_")])

    print(f"{ppi_pair_id} cluster number: {cluster_num}")

    # loop for each cluster
    for i in np.arange(1,cluster_num+1):
        cluster_dir = ppi_pair_dir + "/Cluster_"+str(i)+"/"
        ...
        for j in range(frame_num):
            in_dir = cluster_dir + f"/frame_{j:04d}/"
            out_dir = params["out_FPVec_dir"]+ppi_pair_id+"/"+"Cluster_"+str(i)+"/"+f"/frame_{j:04d}/"
